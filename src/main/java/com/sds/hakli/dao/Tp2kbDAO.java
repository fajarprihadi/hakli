...
public class Tp2kbDAO {
	...
	public List<Vrecp2kb> listRecLetter(String filter, String orderby) throws Exception {
		...
		session = StoreHibernateUtil.openSession();
		oList = session
				.createSQLQuery("SELECT TANGGOTAPK, NOSTR, NOANGGOTA, NAMA, CABANG, SUM(TOTALSKP) AS TOTALSKP FROM TP2KB JOIN TANGGOTA ON TANGGOTAFK = TANGGOTAPK "
						+ "JOIN MCABANG ON MCABANGFK = MCABANGPK WHERE " + filter + " GROUP BY TANGGOTAPK, NOSTR, NOANGGOTA, NOANGGOTA, NAMA, CABANG HAVING SUM(TOTALSKP) > 5 ORDER BY " + orderby)
