...
public class P2kbD02DetailVm {
	...
	@Wire
	private Window winP2kbd02Detail;
	@Wire
	private Grid grid;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tp2kb p2kb,
		...
		grid.setRowRenderer(new RowRenderer<Tp2kbd02>() {
	...
	@NotifyChange("*")
	public void doSubmit(Tp2kbd02 obj, String action, String memotim) {
		try {
	...
	public void doRefresh() {
		try {
			...
			List<Tp2kbd02> objList = oDao.listByFilter(filter, "tp2kbd02pk desc");
	...
	@Command()
	@NotifyChange("*")
	public void doEdit(Tp2kbd02 obj) {
		Map<String, Object> map = new HashMap<>();
		map.put("action", "edit");
		map.put("page", "p2kbd02.zul");
		map.put("p2kb", obj);
		map.put("p2kbkegiatan", obj.getMp2kbkegiatan());
		Event closeEvent = new Event("onClose", winP2kbd02Detail, map);
		Events.postEvent(closeEvent);
	}
	
	@Command()
	@NotifyChange("*")
	public void doDelete(Tp2kbd02 obj) {
			...
			public void onEvent(Event event)
				...
				if (event.getName().equals("onOK")) {
					...
					try {
						...
						doRefresh();
						BindUtils.postNotifyChange(P2kbD02DetailVm.this, "*");
					} catch (Exception e) {
