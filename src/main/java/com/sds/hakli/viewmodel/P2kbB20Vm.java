...
public class P2kbB20Vm {
	
	private org.zkoss.zk.ui.Session zkSession = Sessions.getCurrent();
	private Tanggota anggota;
	private Tp2kbB20DAO oDao = new Tp2kbB20DAO();
	...
	@Wire
	private Window winP2kbb20;
	
	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, 
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kbb20 objForm,
	...
	public BigDecimal getNilaiSkp(Tp2kbb20 obj) throws Exception {
		BigDecimal skp = new BigDecimal(0);
		if (obj.getJeniskegiatan().equals("Pengurangan dan Penanganan Sampah Perundangan") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Pengurangan dan Penanganan Sampah Perundangan)") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Pengurangan dan Penanganan Sampah Perundangan") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Mencegah Kontaminasi dari Penggunaan Zat Kimia") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Mencegah Kontaminasi dari Penggunaan Zat Kimia") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Mencegah Kontaminasi dari Penggunaan Zat Kimia") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Mencegah Pajanan dari Gangguan Fisik Udara") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Mencegah Pajanan dari Gangguan Fisik Udara") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Mencegah Pajanan dari Gangguan Fisik Udara") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		}
		return skp;
	}
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kbb20, map);
	...
	public Tp2kbb20 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kbb20 objForm) {
		this.objForm = objForm;
	}
