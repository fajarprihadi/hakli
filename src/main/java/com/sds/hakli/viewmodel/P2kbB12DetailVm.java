...
public class P2kbB12DetailVm {
	...
	@Wire
	private Window winP2kbb12Detail;
	@Wire
	private Grid grid;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tp2kb p2kb,
		...
		grid.setRowRenderer(new RowRenderer<Tp2kbb12>() {

			@Override
			public void render(Row row, Tp2kbb12 data, int index) throws Exception {
				...
				Label lbl01 = new Label("Judul Kegiatan");
	...
	@NotifyChange("*")
	public void doSubmit(Tp2kbb12 obj, String action, String memotim) {
		try {
			...
			new Tp2kbB12DAO().save(session, obj);
	...
	public void doRefresh() {
		try {
			...
			List<Tp2kbb12> objList = oDao.listByFilter(filter, "tp2kbb12pk desc");
	...
	@Command()
	@NotifyChange("*")
	public void doEdit(Tp2kbb12 obj) {
		Map<String, Object> map = new HashMap<>();
		map.put("action", "edit");
		map.put("page", "p2kbb12.zul");
		map.put("p2kb", obj);
		map.put("p2kbkegiatan", obj.getMp2kbkegiatan());
		Event closeEvent = new Event("onClose", winP2kbb12Detail, map);
		Events.postEvent(closeEvent);
	}
	
	@Command()
	@NotifyChange("*")
	public void doDelete(Tp2kbb12 obj) {
			...
			public void onEvent(Event event)
				...
				if (event.getName().equals("onOK")) {
					...
					try {
						...
						doRefresh();
						BindUtils.postNotifyChange(P2kbB12DetailVm.this, "*");
					} catch (Exception e) {
