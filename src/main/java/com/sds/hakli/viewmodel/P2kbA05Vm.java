...
public class P2kbA05Vm {
	...
	@Wire
	private Window winP2kba05;

	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kba05 objForm,
	...
	public BigDecimal getNilaiSkp(Tp2kba05 obj) throws Exception {
		BigDecimal skp = new BigDecimal(0);
		if (obj.getManfaat().equals("IPTEK Deteksi Dini") && obj.getSpesifikasi().equals("Spesifik")) {
			skp = new BigDecimal(2);
		} else if (obj.getManfaat().equals("IPTEK Deteksi Dini") && obj.getSpesifikasi().equals("Non Spesifik")) {
			skp = new BigDecimal(0);
		} else if (obj.getManfaat().equals("IPTEK Tepat Guna") && obj.getSpesifikasi().equals("Spesifik")) {
			skp = new BigDecimal(3);
		} else if (obj.getManfaat().equals("IPTEK Tepat Guna") && obj.getSpesifikasi().equals("Non Spesifik")) {
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kba05, map);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				...
				String manfaat = (String) ctx.getProperties("manfaat")[0].getValue();
				if (manfaat == null || "".equals(manfaat.trim()))
					this.addInvalidMessage(ctx, "manfaat", Labels.getLabel("common.validator.empty"));
				...
				String spesifikasi = (String) ctx.getProperties("spesifikasi")[0].getValue();
				if (spesifikasi == null || "".equals(spesifikasi.trim()))
					this.addInvalidMessage(ctx, "spesifikasi", Labels.getLabel("common.validator.empty"));
	...
	public Tp2kba05 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kba05 objForm) {
		this.objForm = objForm;
	}
