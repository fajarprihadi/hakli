...
public class P2kbB16Vm {
	
	private org.zkoss.zk.ui.Session zkSession = Sessions.getCurrent();
	private Tanggota anggota;
	private Tp2kbB16DAO oDao = new Tp2kbB16DAO();
	...
	@Wire
	private Window winP2kbb16;
	
	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, 
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kbb16 objForm,
	...
	public void doSave() {
		...
		try {
			...
			if (isInsert) {
				Clients.showNotification("Proses simpan data berhasil", "info", null, "middle_center", 1600);
			} else {
				Clients.showNotification("Proses pembaruan data berhasil", "info", null, "middle_center", 1600);
			}
	...
	public BigDecimal getNilaiSkp(Tp2kbb16 obj) throws Exception {
		BigDecimal skp = new BigDecimal(0);
		if (obj.getJeniskegiatan().equals("KIE (Perbadayaan Masyarakat)") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("KIE (Perbadayaan Masyarakat)") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("KIE (Perbadayaan Masyarakat)") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Pengembangan TTG") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
			...
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Pemeriksaan Kesehatan Penjamah Pangan") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Pemeriksaan Kesehatan Penjamah Pangan") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Pemeriksaan Kesehatan Penjamah Pangan") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		}
		return skp;
	}
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kbb16, map);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				...
				String jenismedia = (String) ctx.getProperties("media")[0].getValue();
				if (jenismedia == null || "".equals(jenismedia.trim()))
					this.addInvalidMessage(ctx, "jenismedia", Labels.getLabel("common.validator.empty"));
	...
	public Tp2kbb16 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kbb16 objForm) {
		this.objForm = objForm;
	}
