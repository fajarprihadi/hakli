				...
				check.addEventListener(Events.ON_CHECK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						...
						if (checked.isChecked()) {
							mapData.put(data.getTp2kba02pk(), data);
						} else {
							mapData.remove(data.getTp2kba02pk());
						}
						totalselected = mapData.size();
					}
				});
				if (mapData.get(data.getTp2kba02pk()) != null)
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									for (Entry<Integer, Tp2kba02> entry : mapData.entrySet()) {
										Tp2kba02 obj = entry.getValue();
