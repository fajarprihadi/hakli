...
public class P2kbA01DetailVm {
	...
	private Map<Integer, Tp2kba01> mapData = new HashMap<>();
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tp2kb p2kb,
		...
		grid.setRowRenderer(new RowRenderer<Tp2kba01>() {
			...
			@Override
			public void render(Row row, Tp2kba01 data, int index) throws Exception {
				...
				check.addEventListener(Events.ON_CHECK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						...
						if (checked.isChecked()) {
							mapData.put(data.getTp2kba01pk(), data);
						} else {
							mapData.remove(data.getTp2kba01pk());
						}
						totalselected = mapData.size();
					}
				});
				if (mapData.get(data.getTp2kba01pk()) != null)
				...
				BindUtils.postNotifyChange(P2kbA01DetailVm.this, "totalskp");
			}
		});
	...
	@Command()
	@NotifyChange("*")
	public void doEdit(Tp2kba01 obj) {
		Map<String, Object> map = new HashMap<>();
		map.put("action", "edit");
		map.put("page", "p2kba01.zul");
		map.put("p2kb", obj);
		map.put("p2kbkegiatan", obj.getMp2kbkegiatan());
		Event closeEvent = new Event("onClose", winP2kba01Detail, map);
		Events.postEvent(closeEvent);
	}

	@Command()
	@NotifyChange("*")
	public void doDelete(Tp2kba01 obj) {
						...
						if (event.getName().equals("onOK")) {
							...
							try {
								...
								Clients.showNotification("Proses hapus data berhasil", "info", null, "middle_center",
										1500);
								doRefresh();
								BindUtils.postNotifyChange(P2kbA01DetailVm.this, "*");
							} catch (Exception e) {
