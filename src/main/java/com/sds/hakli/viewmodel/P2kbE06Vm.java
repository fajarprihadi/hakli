...
public class P2kbE06Vm {
	...
	@Wire
	private Window winP2kbe06;

	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kbe06 objForm,
	...
	public BigDecimal getNilaiSkp(Tp2kbe06 obj) throws Exception {
		BigDecimal skp = new BigDecimal(0);
		if (obj.getJeniskegiatan().equals("Seminar") && obj.getArea().equals("Internasional")) {
			skp = new BigDecimal(1.5);
		} else if (obj.getJeniskegiatan().equals("Seminar") && obj.getArea().equals("Nasional")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Seminar") && obj.getArea().equals("Regional")) {
			skp = new BigDecimal(0.75);
		} else if (obj.getJeniskegiatan().equals("Seminar") && obj.getArea().equals("Lokal")) {
			skp = new BigDecimal(0.5);
		} else if (obj.getJeniskegiatan().equals("Pelatihan / Workshop") && obj.getArea().equals("Internasional")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Pelatihan / Workshop") && obj.getArea().equals("Nasional")) {
			skp = new BigDecimal(1.5);
		} else if (obj.getJeniskegiatan().equals("Pelatihan / Workshop") && obj.getArea().equals("Regional")) {
			skp = new BigDecimal(1.25);
		} else if (obj.getJeniskegiatan().equals("Pelatihan / Workshop") && obj.getArea().equals("Lokal")) {
			skp = new BigDecimal(1);
		}
		return skp;
	}
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kbe06, map);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				...
				String area = (String) ctx.getProperties("area")[0].getValue();
				if (area == null || "".equals(area.trim()))
					this.addInvalidMessage(ctx, "area", Labels.getLabel("common.validator.empty"));
	...
	public Tp2kbe06 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kbe06 objForm) {
		this.objForm = objForm;
	}
