...
public class P2kbE05Vm {
	...
	@Wire
	private Window winP2kbe05;

	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view,
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kbe05 objForm,
	...
	public BigDecimal getNilaiSkp(Tp2kbe05 obj) throws Exception {
		BigDecimal skp = new BigDecimal(0);
		if (obj.getJumlahsoal() >= 50 && obj.getJumlahsoal() <= 100) {
			skp = new BigDecimal(1);
		} else if (obj.getJumlahsoal() > 100 && obj.getJumlahsoal() <= 150) {
			skp = new BigDecimal(2);
		} else if (obj.getJumlahsoal() > 150 && obj.getJumlahsoal() <= 200) {
			skp = new BigDecimal(3);
		} else if (obj.getJumlahsoal() > 200 && obj.getJumlahsoal() <= 250) {
			skp = new BigDecimal(4);
		} else if (obj.getJumlahsoal() > 250 && obj.getJumlahsoal() <= 300) {
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kbe05, map);
	...
	public Tp2kbe05 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kbe05 objForm) {
		this.objForm = objForm;
	}
