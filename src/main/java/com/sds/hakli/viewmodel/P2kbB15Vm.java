...
public class P2kbB15Vm {
	
	private org.zkoss.zk.ui.Session zkSession = Sessions.getCurrent();
	private Tanggota anggota;
	private Tp2kbB15DAO oDao = new Tp2kbB15DAO();
	...
	@Wire
	private Window winP2kbb15;
	
	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, 
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kbb15 objForm,
	...
	public BigDecimal getNilaiSkp(Tp2kbb15 obj) throws Exception {
		BigDecimal skp = new BigDecimal(0);
		if (obj.getJenispenugasan().equals("Penugasan di Lingkup Kesehatan")) {
			skp = new BigDecimal(2);
		} else if (obj.getJenispenugasan().equals("Penugasan di Lingkup Non Kesehatan")) {
			skp = new BigDecimal(2);
		} else if (obj.getJenispenugasan().equals("Penugasan Dalam Situasi Kejadian Luar Biasa (KLB)")) {
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kbb15, map);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String jenispenugasan = (String) ctx.getProperties("jenispenugasan")[0].getValue();
				if (jenispenugasan == null || "".equals(jenispenugasan.trim()))
					this.addInvalidMessage(ctx, "jenispenugasan", Labels.getLabel("common.validator.empty"));
				String pemberitugas = (String) ctx.getProperties("pemberitugas")[0].getValue();
				if (pemberitugas == null || "".equals(pemberitugas.trim()))
					this.addInvalidMessage(ctx, "pemberitugas", Labels.getLabel("common.validator.empty"));
	...
	public Tp2kbb15 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kbb15 objForm) {
		this.objForm = objForm;
	}
