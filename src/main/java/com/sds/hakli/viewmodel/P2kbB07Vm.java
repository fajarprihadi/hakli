...
public class P2kbB07Vm {
	
	private org.zkoss.zk.ui.Session zkSession = Sessions.getCurrent();
	private Tanggota anggota;
	private Tp2kbB07DAO oDao = new Tp2kbB07DAO();
	...
	@Wire
	private Window winP2kbb07;
	
	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, 
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kbb07 objForm,
	...
	public BigDecimal getNilaiSkp(Tp2kbb07 obj) throws Exception {
		...
		if (obj.getJeniskegiatan().equals("Surveilans kualitas media lingkungan") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Surveilans kualitas media lingkungan") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Surveilans kualitas media lingkungan") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Uji Laboratorium dan pengukuran lapangan (insit)") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Uji Laboratorium dan pengukuran lapangan (insit)") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Uji Laboratorium dan pengukuran lapangan (insit)") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Analisa Risiko") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Analisa Risiko") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Analisa Risiko") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kbb07, map);
	...
	public Tp2kbb07 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kbb07 objForm) {
		this.objForm = objForm;
	}
