...
public class P2kbB01DetailVm {
	
	private org.zkoss.zk.ui.Session zkSession = Sessions.getCurrent();
	private Tanggota anggota;
	private Tp2kbB01DAO oDao = new Tp2kbB01DAO();
	private Tp2kbDAO p2kbDao = new Tp2kbDAO();
	
	private Tp2kb p2kb;
	private BigDecimal totalskp;
	
	private Map<Integer, Tp2kbb01> mapData = new HashMap<>();
	...
	@Wire
	private Window winP2kbb01Detail;
	@Wire
	private Grid grid;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tp2kb p2kb,
		...
		grid.setRowRenderer(new RowRenderer<Tp2kbb01>() {

			@Override
			public void render(Row row, Tp2kbb01 data, int index) throws Exception {
				...
				check.addEventListener(Events.ON_CHECK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						...
						if (checked.isChecked()) {
							mapData.put(data.getTp2kbb01pk(), data);
						} else {
							mapData.remove(data.getTp2kbb01pk());
						}
						totalselected = mapData.size();
					}
				});
				if (mapData.get(data.getTp2kbb01pk()) != null)
				...
				BindUtils.postNotifyChange(P2kbB01DetailVm.this, "totalskp");
			}
		});
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									for (Entry<Integer, Tp2kbb01> entry : mapData.entrySet()) {
										Tp2kbb01 obj = entry.getValue();
										obj.setStatus(action);
										new Tp2kbB01DAO().save(session, obj);
									}
	...
	@Command()
	@NotifyChange("*")
	public void doEdit(Tp2kbb01 obj) {
		Map<String, Object> map = new HashMap<>();
		map.put("action", "edit");
		map.put("page", "p2kbb01.zul");
		map.put("p2kb", obj);
		map.put("p2kbkegiatan", obj.getMp2kbkegiatan());
		Event closeEvent = new Event("onClose", winP2kbb01Detail, map);
		Events.postEvent(closeEvent);
	}
	
	@Command()
	@NotifyChange("*")
	public void doDelete(Tp2kbb01 obj) {
			...
			public void onEvent(Event event)
				...
				if (event.getName().equals("onOK")) {
					...
					try {
						...
						doRefresh();
						BindUtils.postNotifyChange(P2kbB01DetailVm.this, "*");
					} catch (Exception e) {
