...
public class P2kbB14Vm {
	
	private org.zkoss.zk.ui.Session zkSession = Sessions.getCurrent();
	private Tanggota anggota;
	private Tp2kbB14DAO oDao = new Tp2kbB14DAO();
	...
	@Wire
	private Window winP2kbb14;
	
	@AfterCompose
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, 
			@ExecutionArgParam("obj") Mp2kbkegiatan p2kb, @ExecutionArgParam("objForm") Tp2kbb14 objForm,
	...
	public BigDecimal getNilaiSkp(Tp2kbb14 obj) throws Exception {
		BigDecimal skp = new BigDecimal(0);
		if (obj.getJeniskegiatan().equals("Air: Filtrasi, Sedimentasi, Airasi, Dekontaminasi") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Air: Filtrasi, Sedimentasi, Airasi, Dekontaminasi") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Air: Filtrasi, Sedimentasi, Airasi, Dekontaminasi") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Pangan: KIE, Rekayasa Teknologi Pengolahan Pangan") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Pangan: KIE, Rekayasa Teknologi Pengolahan Pangan") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Pangan: KIE, Rekayasa Teknologi Pengolahan Pangan") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		} else if (obj.getJeniskegiatan().equals("Sarana dan Bangunan: KIE dan Pengembangan TTG") && obj.getStatuspeserta().equals("Ketua Tim")) {
			skp = new BigDecimal(3);
		} else if (obj.getJeniskegiatan().equals("Sarana dan Bangunan: KIE dan Pengembangan TTG") && obj.getStatuspeserta().equals("Anggota Tim")) {
			skp = new BigDecimal(2);
		} else if (obj.getJeniskegiatan().equals("Sarana dan Bangunan: KIE dan Pengembangan TTG") && obj.getStatuspeserta().equals("Kontributor")) {
			skp = new BigDecimal(1);
		}
		return skp;
	}
	...
	public void doClose() {
		...
		Event closeEvent = new Event("onClose", winP2kbb14, map);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				...
				String namamedia = (String) ctx.getProperties("media")[0].getValue();
				if (namamedia == null || "".equals(namamedia.trim()))
					this.addInvalidMessage(ctx, "namamedia", Labels.getLabel("common.validator.empty"));
	...
	public Tp2kbb14 getObjForm() {
		return objForm;
	}

	public void setObjForm(Tp2kbb14 objForm) {
		this.objForm = objForm;
	}
