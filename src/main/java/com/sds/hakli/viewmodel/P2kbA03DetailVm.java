...
public class P2kbA03DetailVm {
	...
	private Map<Integer, Tp2kba03> mapData = new HashMap<>();
	...
	@Wire
	private Window winP2kba03Detail;
	@Wire
	private Grid grid;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tp2kb p2kb,
		...
		grid.setRowRenderer(new RowRenderer<Tp2kba03>() {
			...
			@Override
			public void render(Row row, Tp2kba03 data, int index) throws Exception {
				...
				check.addEventListener(Events.ON_CHECK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						...
						if (checked.isChecked()) {
							mapData.put(data.getTp2kba03pk(), data);
						} else {
							mapData.remove(data.getTp2kba03pk());
						}
						totalselected = mapData.size();
					}
				});
				if (mapData.get(data.getTp2kba03pk()) != null)
				...
				BindUtils.postNotifyChange(P2kbA03DetailVm.this, "totalskp");
			}
		});
						...
						public void onEvent(Event event) throws Exception {
							if (event.getName().equals("onOK")) {
								try {
									...
									for (Entry<Integer, Tp2kba03> entry : mapData.entrySet()) {
										Tp2kba03 obj = entry.getValue();
										obj.setStatus(action);
										new Tp2kbA03DAO().save(session, obj);
									}
									...
									Event closeEvent = new Event("onClose", winP2kba03Detail, null);
	...
	public void doRefresh() {
		try {
			...
			List<Tp2kba03> objList = oDao.listByFilter(filter, "tp2kba03pk desc");
	...
	@Command()
	@NotifyChange("*")
	public void doEdit(Tp2kba03 obj) {
		Map<String, Object> map = new HashMap<>();
		map.put("action", "edit");
		map.put("page", "p2kba03.zul");
		map.put("p2kb", obj);
		map.put("p2kbkegiatan", obj.getMp2kbkegiatan());
		Event closeEvent = new Event("onClose", winP2kba03Detail, map);
		Events.postEvent(closeEvent);
	}
	
	@Command()
	@NotifyChange("*")
	public void doDelete(Tp2kba03 obj) {
			...
			public void onEvent(Event event)
				...
				if (event.getName().equals("onOK")) {
					...
					try {
						...
						doRefresh();
						BindUtils.postNotifyChange(P2kbA03DetailVm.this, "*");
					} catch (Exception e) {
