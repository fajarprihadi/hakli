...
public class P2kbC01DetailVm {
	...
	@Wire
	private Window winP2kbc01Detail;
	@Wire
	private Grid grid;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view, @ExecutionArgParam("obj") Tp2kb p2kb,
		...
		grid.setRowRenderer(new RowRenderer<Tp2kbc01>() {

			@Override
			public void render(Row row, Tp2kbc01 data, int index) throws Exception {
									...
									Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener<Event>() {
										...
										public void onEvent(Event event) throws Exception {
											if (event.getName().equals("onOK")) {
												...
												BindUtils.postNotifyChange(P2kbC01DetailVm.this, "totalskp");
											}
										}
									});
						} else {
				...
				Div divKegiatan0 = new Div();
				divKegiatan0.setSclass("note note-light");
				Label lbl01 = new Label("Nama Kegiatan");
				lbl01.setStyle("font-weight: bold");
				divKegiatan0.appendChild(lbl01);
				Label lbl02 = new Label(": " + data.getJudul());
				divKegiatan0.appendChild(lbl02);
				vlayoutKegiatan.appendChild(divKegiatan0);
				...
				Label lbl1 = new Label("Jenis Kegiatan");
				lbl1.setStyle("font-weight: bold");
				divKegiatan1.appendChild(lbl1);
				Label lbl2 = new Label(": " + data.getJeniskegiatan());
				...
				Label lbl3 = new Label("Status Dalam Kepesertaan");
	...
	@NotifyChange("*")
	public void doSubmit(Tp2kbc01 obj, String action, String memotim) {
		try {
			...
			new Tp2kbC01DAO().save(session, obj);
...
//					+ p2kb.getTp2kbbook().getTglmulai() + "' and '" + p2kb.getTp2kbbook().getTglakhir() + "')";
			...
			List<Tp2kbc01> objList = oDao.listByFilter(filter, "tp2kbc01pk desc");
	...
	@Command()
	@NotifyChange("*")
	public void doEdit(Tp2kbc01 obj) {
		Map<String, Object> map = new HashMap<>();
		map.put("action", "edit");
		map.put("page", "p2kbc01.zul");
		map.put("p2kb", obj);
		map.put("book", p2kb.getTp2kbbook());
		map.put("p2kbkegiatan", obj.getMp2kbkegiatan());
		Event closeEvent = new Event("onClose", winP2kbc01Detail, map);
		Events.postEvent(closeEvent);
	}

	@Command()
	@NotifyChange("*")
	public void doDelete(Tp2kbc01 obj) {
						...
						if (event.getName().equals("onOK")) {
							...
							try {
								...
								Clients.showNotification("Proses hapus data berhasil", "info", null, "middle_center",
										1500);
								doRefresh();
								BindUtils.postNotifyChange(P2kbC01DetailVm.this, "*");
							} catch (Exception e) {
